...
pbp_2017 %>% 
  ...
  select(time,TimeSecs,qtr,down,ydstogo,yrdln,yrdline100,ydsnet,posteam,DefensiveTeam,PlayType,Yards.Gained,AirYards,YardsAfterCatch,desc) %>% View()
```

```{r no_play}
# Explore Type of No Plays that are non-PassAttempts -- potentially penalties?
pbp_2017 %>% 
  filter(PlayType == "No Play" & PassAttempt == 0) %>% 
  select(starts_with("Pen"),desc) %>% View()
